function visitBalancePage() {
    ...
    function handleEconomyData(data) {
        ...
        let nextBalanceTime = 86400 - (Date.now() / 1000 - lastClaimedBalance);
        nextBalanceTimeElement.textContent = formatTimeRemaining(nextBalanceTime);
        ...
        if (activeMembership) {
            ...
            const daysAgo = Math.floor((Date.now() / 1000 - membershipStarted) / 86400);
            membershipStartedElement.textContent = daysAgo === 0 ? "Today" : `${daysAgo} Day${daysAgo > 1 ? 's' : ''} Ago`;

            const timeLeft = membershipEnding - (Date.now() / 1000);
            membershipEndingElement.textContent = formatTimeRemaining(timeLeft);
                ...
                .then(data => {
                    if (data && data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: data.message || 'Response from API!',
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Oopsie!',
                            text: data.error || 'API response error.',
