function visitRegisterPage(queryParams) {
    ...
    if (code && state) {
        ...
        document.querySelector('.container').innerHTML = `
            <div id="login-section" class="middle-float">
                <h2 class="text-center">Sign Up for Clippsly</h2>
                <h4 class="acknowledge-checkbox">
                    <label>By creating an account you are agreeing to Clippsly's <b><a data-global-target="openExternal" inline class="hyperlink" href="https://clippsly.com/terms-of-use">Terms of Use</a></b> and <b><a data-global-target="openExternal" inline class="hyperlink" href="https://clippsly.com/privacy-policy">Privacy Policy</a></b>.</label>
                </h4>
                ...
                <h4>Already have an account? <a inline class="hyperlink" href="/login">Sign In</a></h4>
            </div>
        `
...
function finalizeOAuthRegister(code, state) {
    $.ajax({
        ...
        success: function (response) {
            console.log(
                'OAuth Registration Finalization Response (raw):',
                response
            )
            ...
            if (parsedResponse.session_token) {
                ...
                showRegisterFailedMessage(
                    'Registration failed. No session token was provided.'
                )
            }
        },
...
function initiateOAuthRegister(platform) {
    $.ajax({
        ...
        success: function (response) {
            console.log(
                'OAuth Register API Response (raw):',
                response
            )
